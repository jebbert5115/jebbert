...
const bots: Bot[] = [
  {
    ...
    website: 'https://nexus.jebbert.xyz',
  },
  {
    name: 'HUDdy',
...
export default function Projects() {
  return (
    <div className="projects-list">
      {bots.map((bot) => (
        <div key={bot.name} className="project-row">
          <img src={bot.logo} alt={`${bot.name} logo`} className="project-logo" />
          ...
          <div className="project-info">
            <div className="project-name-row">
              {bot.website ? (
                ...
                <span className="project-name">{bot.name}</span>
              )}
              <span className="bot-lang-tag" style={{ borderColor: langColor[bot.lang], color: langColor[bot.lang] }}>
                {bot.lang}
              </span>
            </div>
            <p className="project-about">{bot.about}</p>
          </div>

          <div className="project-action">
            {bot.invite ? (
              <a className="bot-invite-btn" href={bot.invite} target="_blank" rel="noreferrer">
